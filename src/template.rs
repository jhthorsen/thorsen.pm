...
fn template_filter_match(value: &tera::Value, args: &HashMap<String, tera::Value>) -> Result<tera::Value, Error> {
    let re = Regex::new(args.get("re").unwrap().as_str().unwrap()).unwrap();
    Ok(to_value(re.is_match(value.as_str().unwrap()))?)
}

fn template_filter_markdown(value: &tera::Value, _args: &HashMap<String, tera::Value>) -> Result<tera::Value, Error> {
...
fn template_function_qs(args: &HashMap<String, tera::Value>) -> Result<tera::Value, Error> {
        ...
        for (key, value) in orig.iter() {
            if ! args.contains_key(key) {
                if let Some(v) = value.as_str() {
                    path = format!("{}{}{}={}", path, if path.len() > 0 { "&" } else { "" }, key, v);
                }
            }
        }
    }
    ...
    for key in args.keys() {
        if let Some(tera::Value::String(value)) = args.get(key) {
            path = format!("{}{}{}={}", path, if path.len() > 0 { "&" } else { "" }, key, value.as_str());
        }
    }
